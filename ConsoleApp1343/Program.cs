static class Program
{
    ...
    {
        Console.WriteLine($"Найти сумму: {FindSum(12)}");
        var dic = CalcNatur(100);
        foreach (var item in dic)
            Console.WriteLine(item);
        Console.WriteLine($"Отсортировать число: {SortNum(987654321)}");
        Console.WriteLine($"Пирамида: {FindPyramidSum(1)}");
        Console.WriteLine($"Вернуть 5: {ReturnFive()}");
    }
    // 4 задание
